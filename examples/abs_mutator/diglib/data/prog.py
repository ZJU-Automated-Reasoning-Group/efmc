...
class Prog:
    ...
    def get_traces(self, inps: diglib.data.traces.Inps):
        ...
        traces = diglib.data.traces.DTraces.parse(traces, self.inv_decls)
    ...
    # PRIVATE METHODS
    @beartype
    def _get_traces(self, inp:diglib.data.traces.Inp):
    ...
    @beartype
    def _get_traces_mp(self, inps:diglib.data.traces.Inps):
        """
    ...
    def _get_valid_inp_ranges(self):
        ...
        for inp_range in inp_ranges:
            ...
            my_inp = diglib.data.traces.Inp(self.inp_decls.names, inp)
            dr[my_inp] = inp_range
            di[my_inp] = inp

        my_inps = diglib.data.traces.Inps(di.keys())
...
class Src(metaclass=abc.ABCMeta):
    ...
    @beartype
    def __init__(self, filename:Path, tmpdir:Path):
