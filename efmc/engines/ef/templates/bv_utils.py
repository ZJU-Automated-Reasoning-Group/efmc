...
def get_signedness(formula: z3.BitVecRef) -> Signedness:
    """Get the signedness of a bit-vector formula.
    FIXME: this is generated by LLM; need to check if it is correct.
    """
    if formula.size() == 1:
        return Signedness.UNKNOWN

    sign_bit = Extract(formula.size() - 1, formula.size() - 1, formula)
    if z3.is_true(z3.simplify(sign_bit == 1)):
