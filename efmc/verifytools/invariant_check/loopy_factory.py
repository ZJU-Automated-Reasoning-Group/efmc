...
class LoopyFactory:
    ...
    def get_benchmark(self, benchmarks_file="", features=None, no_preprocess=False) -> Benchmark:
        if self.name == "frama-c":
            return FramaCBenchmark(benchmarks_file, features, no_preprocess)
        elif self.name == "boogie":
            return BoogieBenchmark(benchmarks_file, features, no_preprocess)
        else:
