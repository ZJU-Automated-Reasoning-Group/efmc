...
class TestBDDProver(TestCase):
    ...
    def test_performance_comparison(self):
        """Compare performance of BDD prover vs PDR prover on a complex Boolean system"""
        print("\nPerformance comparison: BDD vs PDR")
        
        # Create the transition system
        sts = TransitionSystem()
        sts.from_z3_cnts(list(get_complex_bool_sys()))
        
        # Test BDD prover (forward)
        print("Running BDD prover (forward)...")
        bdd_start = time.time()
        bdd_prover = BDDProver(sts, use_forward=True)
        bdd_result = bdd_prover.solve()
        bdd_time = time.time() - bdd_start
        print(f"BDD prover (forward) time: {bdd_time:.4f} seconds")
        print(f"BDD result: {'Safe' if bdd_result.is_safe else 'Unsafe'}")
        
        # Test BDD prover (backward)
        print("Running BDD prover (backward)...")
        bdd_back_start = time.time()
        bdd_back_prover = BDDProver(sts, use_forward=False)
        bdd_back_result = bdd_back_prover.solve()
        bdd_back_time = time.time() - bdd_back_start
        print(f"BDD prover (backward) time: {bdd_back_time:.4f} seconds")
        print(f"BDD result: {'Safe' if bdd_back_result.is_safe else 'Unsafe'}")
        
        # Test PDR prover
        print("Running PDR prover...")
        pdr_start = time.time()
        pdr_prover = PDRProver(sts)
        pdr_result = pdr_prover.solve()
        pdr_time = time.time() - pdr_start
        print(f"PDR prover time: {pdr_time:.4f} seconds")
        print(f"PDR result: {'Safe' if pdr_result.is_safe else 'Unsafe'}")
        
        # Compare results
        self.assertEqual(bdd_result.is_safe, pdr_result.is_safe, 
                         "BDD and PDR provers should agree on safety")
        
        print(f"BDD (forward) is {pdr_time/bdd_time:.2f}x {'faster' if bdd_time < pdr_time else 'slower'} than PDR")
        print(f"BDD (backward) is {pdr_time/bdd_back_time:.2f}x {'faster' if bdd_back_time < pdr_time else 'slower'} than PDR")
    
    def test_scalability(self):
        """Test the scalability of the BDD prover with larger systems"""
        print("\nScalability test for BDD prover")
        
        # Test with different sizes of the counter system
        for n in [4, 6, 8]:
            print(f"\nTesting with {n}-bit counter (state space size: 2^{n} = {2**n})")
            
            # Create the transition system
            sts = TransitionSystem()
            sts.from_z3_cnts(list(get_large_bool_sys(n)))
            
            # Test BDD prover (forward)
            print(f"Running BDD prover (forward) on {n}-bit counter...")
            bdd_start = time.time()
            bdd_prover = BDDProver(sts, use_forward=True)
            bdd_result = bdd_prover.solve()
            bdd_time = time.time() - bdd_start
            print(f"BDD prover time: {bdd_time:.4f} seconds")
            print(f"BDD result: {'Safe' if bdd_result.is_safe else 'Unsafe'}")
            
            # Note: The counter system might be found unsafe by the general algorithm
            # due to the way the property is formulated.
            if not bdd_result.is_safe:
                print("Note: The counter system was found to be unsafe by the general algorithm.")
        
        # Test with token ring protocol
        for n in [3, 4, 5]:
            print(f"\nTesting with token ring protocol ({n} processes)")
            
            # Create the transition system
            sts = TransitionSystem()
            sts.from_z3_cnts(list(get_token_ring_sys(n)))
            
            # Test BDD prover (forward)
            print(f"Running BDD prover (forward) on token ring with {n} processes...")
            bdd_start = time.time()
            bdd_prover = BDDProver(sts, use_forward=True)
            bdd_result = bdd_prover.solve()
            bdd_time = time.time() - bdd_start
            ...
            # Note: The token ring system might be found unsafe by the general algorithm
            # due to the way the property is formulated.
            if not bdd_result.is_safe:
                print("Note: The token ring system was found to be unsafe by the general algorithm.")
