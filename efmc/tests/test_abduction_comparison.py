...
def compare_abductions(pre_cond, post_cond, qe_result, dillig_result):
    ...
    if dillig_result is not None:
        result['dillig_consistency'], result['dillig_sufficiency'] = validate_abduction(pre_cond, post_cond, dillig_result)
...
class TestAbductionComparison(TestCase):
    ...
    def run_comparison(self, name, pre_cond, post_cond):
        ...
        print(f"Dillig-style abduction: {'Success' if comparison['dillig_success'] else 'Failed'} in {dillig_time:.4f}s")
    ...
    def test_unsatisfiable(self):
        ...
        self.assertFalse(comparison['dillig_success'], "Dillig-style abduction should fail with unsatisfiable implication")
