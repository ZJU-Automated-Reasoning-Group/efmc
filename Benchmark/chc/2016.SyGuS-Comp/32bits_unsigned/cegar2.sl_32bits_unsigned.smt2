(set-logic HORN)
(declare-fun inv ((_ BitVec 32) (_ BitVec 32) (_ BitVec 32) ) Bool)
(assert (forall ((x (_ BitVec 32)) (n (_ BitVec 32)) (m (_ BitVec 32))) 
       (=> ( and ( = x (_ bv0 32) ) ( = m (_ bv0 32) ) ) (inv x n m))))
(assert (forall ((x (_ BitVec 32)) (n (_ BitVec 32)) (m (_ BitVec 32)) (x! (_ BitVec 32)) (n! (_ BitVec 32)) (m! (_ BitVec 32))) 
       (=> (and (inv x n m) ( let ( ( a!1 ( and ( bvult x n ) ( = x! ( bvadd x (_ bv1 32) ) ) ( = n! n ) ) ) ) ( or ( and a!1 ( = m! m ) ) ( and a!1 ( = m! x ) ) ) )) (inv x! n! m!))))
(assert (forall ((x (_ BitVec 32)) (n (_ BitVec 32)) (m (_ BitVec 32))) 
       (=> (inv x n m) ( not ( and ( bvuge x n ) ( bvugt n (_ bv0 32) ) ( or ( bvule n m ) ( bvult m (_ bv0 32) ) ) ) ))))
(check-sat)
