...
def trans2smt(filename):
    ...
    ini = re.findall('Initial\s*\{([^}]*)\}', content, re.S)
    ini_str = ini[0].strip() if ini else ""

    bad = re.findall('Bad\s*\{([^}]*)\}', content, re.S)
    bad_str = bad[0].strip() if bad else ""

    trans = re.findall('Transition\s*\{([^}]*)\}', content, re.S)
    trans_str = trans[0].strip() if trans else ""

    # Add extraction for P0 and P1
    p0 = re.findall('P0\s*\{([^}]*)\}', content, re.S)
    p0_str = p0[0].strip() if p0 else ""

    p1 = re.findall('P1\s*\{([^}]*)\}', content, re.S)
    p1_str = p1[0].strip() if p1 else ""

    # Convert components to SMT format
    ini_smt = AutomatonModel(ini_str).ini_to_regex() if ini_str else ""
    bad_smt = AutomatonModel(bad_str).bad_to_regex() if bad_str else ""
    trans_smt = AutomatonModel(trans_str).to_transducer() if trans_str else ""
    p0_smt = AutomatonModel(p0_str).ini_to_regex() if p0_str else ""
    p1_smt = AutomatonModel(p1_str).ini_to_regex() if p1_str else ""
...
if __name__ == '__main__':
    ...
    output_name = "../models/smt2/linear/game_examples/" + filename[:-4] + ".smt2"
    f = open(output_name, "w")
    f.write("(set-logic HORN)\n\n(declare-fun Acc (String) Bool)\n\n")
    f.write(trans2smt('../models/games/' + filename))
    f.write("\n\n(check-sat)")
    f.close()
