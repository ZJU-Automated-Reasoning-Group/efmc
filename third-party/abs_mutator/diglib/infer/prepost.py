...
import diglib.infer.base
from diglib.data.traces import Traces, DTraces
from diglib.data.inv.base import Inv
from diglib.data.inv.invs import Invs, DInvs
from diglib.data.inv.eqt import Eqt
from diglib.data.inv.oct import Oct
from diglib.data.inv.prepost import PrePost

DBG = pdb.set_trace
mlog = CM.getLogger(__name__, settings.LOGGER_LEVEL)


class Infer(diglib.infer.base.Infer):
    def __init__(self, symstates, prog):
        super().__init__(symstates, prog)
        self.use_reals = self.symstates.use_reals

    def gen(self, dinvs, dtraces):
        ...
        for loc in post_locs:
            ...
            for postcond in postconds:
                prepost = self.get_preposts(loc, postcond, dtraces)
                if prepost:
    ...
    def get_preposts(self, loc, postcond, dtraces):
        assert isinstance(loc, str), loc
        assert postcond.operator() == operator.eq, postcond

        postcond_expr = Eqt(postcond).expr(self.use_reals)

        import diglib.infer.opt
        solver = diglib.infer.opt.Ieq(self.symstates, self.prog)
        preconds = solver.gen(dtraces, [loc], postcond_expr)

        preconds = list(preconds[loc]) if loc in preconds else []
        print('hi', postcond, preconds)
