...
def iread_strip(filename: str, strip_c: Optional[str]='#'):
    """
...
def getLogger(name: str, level: int):
...
def vload(filename: str, mode: str ='rb'):
    with open(filename, mode) as fh:
        import pickle
        sobj = pickle.load(fh)
    return sobj
