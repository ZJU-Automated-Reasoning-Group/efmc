...
class SymbsValsSet(set):
    ...
    @beartype
    def __contains__(self, t: SymbsVals):
        return super().__contains__(t)

    @beartype
    def add(self, t: SymbsVals):
        return super().add(t)
...
class Traces(SymbsValsSet):
    ...
    @beartype
    def __str__(self, printDetails: bool=False):
        if printDetails:
...
class DTraces(dict):
    ...
    @beartype
    def add(self, loc: str, trace: Trace):
        if loc not in self:
            self[loc] = Traces()
    ...
    @beartype
    def vwrite(self, inv_decls, tracefile: Path):
        """
        ...
        ...
        """
        assert inv_decls and isinstance(inv_decls,diglib.data.prog.DSymbs), inv_decls
    ...
    def vread(cls, tracefile: Path):
        ...
        with open(tracefile) as csvfile:
            ...
            inv_decls =diglib.data.prog.DSymbs()
            ...
            for row in myreader:
                ...
                if loc not in inv_decls:
                    inv_decls[loc] =diglib.data.prog.Symbs.mk(contents)
                else:
