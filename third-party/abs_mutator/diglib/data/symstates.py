...
class PathCondCIVL(PathCond):
    ...
    @beartype
    @classmethod
    def parse_part(cls, symstates: list):
        """
        ['vtrace1: q = 0; r = X_x; a = 0; b = 0; x = X_x; y = X_y',
        'path condition: (0<=(X_x-1))&&(0<=(X_y-1))']
        """
...
class PathCondJPF(PathCond):
    ...
    @beartype
    @classmethod
    def parse_parts(cls, lines, delim:str="**********"):
        """
...
class PCs(set):  #{PathConds}
    ...
    @beartype
    def __init__(self, loc:str, depth:int, read_from_file:bool = False):
...
class SymStates(dict):
    ...
    @beartype
    def __init__(self, inp_decls: diglib.data.prog.Symbs , inv_decls: diglib.data.prog.DSymbs):
    ...
    def compute(self, symstatesmaker_cls,
                filename: Path, mainQName: str, funname: str, tmpdir:Path):
    ...
    @beartype
    def vwrite(self, sstatesfile: Path):
        """
    ...
    def check(self, dinvs: diglib.infer.inv.DInvs, 
        ...
        Also update inps
        """
        # assert not inps or (isinstance(inps, diglib.data.traces.Inps) and inps), inps
        ...
        mCexs = []
        mdinvs = diglib.infer.inv.DInvs()
        for loc, str_inv, (cexs, is_succ) in wrs:
            inv = refsD[(loc, str_inv)]

            if cexs:
                stat = diglib.infer.inv.Inv.DISPROVED
                mCexs.append({loc: {str(inv): cexs}})
            else:
                stat = (
                    diglib.infer.inv.Inv.PROVED if is_succ else diglib.infer.inv.Inv.UNKNOWN
                )
            inv.stat = stat
            mdinvs.setdefault(loc, diglib.infer.inv.Invs()).add(inv)
    ...
    def mcheck_depth(self, ssd: SymStatesDepth,
        ...
        def f(depth):
            ...
            self.put_solver_stats(diglib.analysis.CheckSolverCalls(stat))
        ...
        if stat != z3.unsat:  # if disprove (sat) or unknown first time
            self.put_solver_stats(
                diglib.analysis.CheckDepthChanges(
    ...
    def mcheck(self, symstates_expr: z3.ExprRef,
        ...
        #assert inps is None or isinstance(inps, diglib.data.traces.Inps), inps
    ...
    @beartype
    def maximize(self, loc:str, term_expr:z3.ExprRef, iupper: int):
        """
    ...
    def mmaximize_depth(self, ssd:SymStatesDepth , 
        ...
        def f(depth: int):
            ...
            self.put_solver_stats(diglib.analysis.MaxSolverCalls(stat))
            return maxv, stat
        ...
        if maxv is not None:  # if changed
            self.put_solver_stats(
                diglib.analysis.MaxDepthChanges(
    ...
    def get_inp_constrs(self, inps):
        cstrs = []
        if isinstance(inps, diglib.data.traces.Inps) and inps:
...
class SymStatesMaker(metaclass=abc.ABCMeta):
    ...
    @beartype
    @classmethod
    def merge(cls, depthss: list, pc_cls):
        """
