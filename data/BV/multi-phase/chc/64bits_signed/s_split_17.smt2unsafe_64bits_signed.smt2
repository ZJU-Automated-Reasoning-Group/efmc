...
(assert (forall ((v (_ BitVec 64))
  ...
  (let ((a!1 (and (inv v x z w)
                  ...
                  (= v! (ite (bvslt x z) (bvadd v #x0000000000000001) v))
                  (= w! (ite (bvslt x z) w (bvadd w #x0000000000000001))))))
    (=> a!1 (inv x! z! v! w!)))))
(assert (forall ((v (_ BitVec 64))
         (x (_ BitVec 64))
         (z (_ BitVec 64))
         (w (_ BitVec 64)))
  (let ((a!1 (not (and (not (bvsle v #x00000000000003e8))
                       (not (bvsle w #x0000000000000000))))))
    (=> (inv v x z w) a!1))))
(check-sat)
