...
; Initial condition: x is in [0.0, 5.0]
(define-fun init () Bool
  (and (fp.geq x ((_ to_fp 8 24) RNE 0.0))
       (fp.leq x ((_ to_fp 8 24) RNE 5.0))))

; Transition: x' = x + 2.0 (always increments)
(define-fun trans () Bool
  (fp.eq x! (fp.add RNE x ((_ to_fp 8 24) RNE 2.0))))

; Post condition: x should remain <= 5.0 (will be violated)
